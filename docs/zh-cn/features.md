...
地球图标按钮 + CSS 下拉菜单，当站点配置了 2+ 种语言时自动显示。

### 配置

i18n 功能无需特殊配置，只需 Hugo 多语言配置。

### 用法

当站点配置了 2+ 种语言时，语言切换按钮会自动出现在页面头部。
按钮使用 Hugo 的 `.Page.Translations` 链接到翻译版本。
对于系列/分类术语页，系统会特殊处理不同语言 slug 的情况，通过文章翻译链进行关联。
支持键盘操作，可通过 Tab 键导航和 Enter 键选择。

---

## 系列

系列分类法，列表页 (/series/) 以卡片形式展示所有系列，详情页以步骤/时间线布局展示。
...
系列分类法需要在 Hugo 配置的 taxonomies 中启用：
```toml
[taxonomies]
  series = "series"
```

在文章的 front matter 中添加 weight 和 series：
```yaml
---
title: "文章标题"
date: 2024-01-15
...
详情页 (/series/系列名称/) 默认按倒序排列（最新在前），可通过排序切换按钮改为正序（阅读顺序）。
用户偏好通过 localStorage 持久化（series-sort.js）。
双语系列使用语言特定名称：中文 "eBPF 可观测性系列"，英文 "eBPF Observability Series"。
